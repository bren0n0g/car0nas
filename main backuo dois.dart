  ...
  for (var viagem in viagens) {
    ...
    if (regra.tipoDia == 'Dia Anterior') {
      ...
      if (viagem.dia == regra.diaFixo) {
        if (isManual || hojeStr == regra.diaFixo) {
          adicionarViagem = true;
        }
      }
    }

    // 2. CHECAGEM DE IDA/VOLTA (Totalmente limpo, sem o legado 'Ambos')
    if (adicionarViagem && regra.alvoCarona != viagem.tipo) {
      adicionarViagem = false;
    }

    // 3. CHECAGEM DE HORÁRIO (Apenas automático)
...
Future<int> executarRegra(RegraMensagem regra, List<Viagem> viagens, {bool isManual = false}) async {
  ...
  if (WhatsappService.client == null) {
    return -1; // Sem sessão conectada
  }

  if (destinatarios.isEmpty) {
    return 0; // Ninguém bate com o filtro hoje
  }
  ...
  // Falha na sincronia de chaves criptográficas (Hydration Lock)
  if (enviosComSucesso == 0 && destinatarios.isNotEmpty) {
    return -2; 
  }
  ...
  for (int i = 0; i < regras.length; i++) {
    ...
    if (regra.tipoHorario == 'Horário Específico') {
      ...
      for (var v in viagens) {
        // Alarme respeita estritamente Ida ou Volta
...
void main() async {
  ...
  // TRAVA DE HARDWARE: Mantém o app blindado na vertical
...
class RegraMensagem {
  ...
  factory RegraMensagem.fromJson(Map<String, dynamic> json) => RegraMensagem(
    texto: json['texto'], 
...
// ==========================================
// TELA DE NAVEGAÇÃO COM INDEXED STACK (PARALELISMO)
// ==========================================
...
class _TelaNavegacaoState extends State<TelaNavegacao> {
  ...
  Future<void> _carregarDados() async {
    ...
    setState(() {
      if (prefs.containsKey('contatos')) {
        bancoContatos = List<Pessoa>.from(jsonDecode(prefs.getString('contatos')!).map((m) => Pessoa.fromJson(m)));
      }
      ...
      if (prefs.containsKey('regras')) {
        ...
        // MIGRADOR DE LEGADO: Limpa lixo antigo do "Ambos" que quebra o app
        for (var r in bancoRegras) {
          if (r.alvoCarona == 'Ambos' || r.alvoCarona == null) {
            r.alvoCarona = 'Ida';
          }
        }
      }
      ...
      if (prefs.containsKey('viagens')) {
        viagens = List<Viagem>.from(jsonDecode(prefs.getString('viagens')!).map((m) => Viagem.fromJson(m)));
      }
  ...
  @override
  Widget build(BuildContext context) {
    // AS 4 ABAS AGORA SÃO IMORTAIS GRAÇAS AO INDEXED STACK
    return Scaffold(
      body: SafeArea(
        child: IndexedStack(
          index: _indiceAtual,
          children: [
            ...
            // A Aba do WhatsApp roda aqui, e o estado dela nunca se perde!
...
// ==========================================
// ABA: WHATSAPP CONNECTION (PARALELA E ESTÁVEL)
// ==========================================
...
class _WhatsAppConnectionTabState extends State<WhatsAppConnectionTab> {
  ...
  Future<void> _limparSessaoZumbi() async {
    if (WhatsappService.client != null) {
      ...
      await Future.delayed(const Duration(seconds: 1)); // Dá tempo da RAM esvaziar
    }
  }
  ...
  Future<void> _iniciarProcessoDeConexao() async {
    ...
    if (_isLoading) return; // Trava contra duplo clique

    FocusScope.of(context).unfocus(); 
    String numeroFormatado = formatarTelefoneBr(_phoneController.text);
    ...
    while (_loopAtivo) {
      try {
        if (mounted) {
          setState(() {
            WhatsappService.status = "Iniciando..."; 
            WhatsappService.pairingCode = "";
            WhatsappService.qrCodeImage = null;
          });
        }
        ...
        WhatsappService.client = await WhatsappBotFlutterMobile.connect(
          ...
          onPhoneLinkCode: (String code) {
            if (!_loopAtivo || !mounted) return; 
            setState(() {
          ...
          onQrCode: (String qr, Uint8List? imageBytes) {
            ...
            setState(() {
              if (WhatsappService.pairingCode.isEmpty) {
                WhatsappService.status = "AGUARDANDO LEITURA (QR)";
                WhatsappService.qrCodeImage = imageBytes;
              }
            });
          },
        );
        
        // Se a função conectou sem dar Crash, quebramos o loop! Ele está logado com sucesso.
        break; 
        ...
        if (mounted) {
          setState(() { 
            WhatsappService.status = "ERRO DE CONEXÃO. TENTANDO NOVAMENTE..."; 
          });
        }
        await Future.delayed(const Duration(seconds: 4)); // Pausa de segurança
      }
    }
  ...
  Future<void> _desconectarWhatsApp() async {
    ...
    if (mounted) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Operação encerrada com sucesso!')));
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    // Agora o "emSessao" só ativa se o aplicativo conectou E não gerou código de pareamento.
    // Isso garante que se o código aparecer, a caixa ficará visível imortalmente!
    bool emSessao = WhatsappService.numeroConectado.isNotEmpty && WhatsappService.status == "CONECTADO" && WhatsappService.pairingCode.isEmpty;

    Color corDoStatus;
    if (WhatsappService.status == "CONECTADO") {
      corDoStatus = Colors.green;
    } else if (WhatsappService.status == "Desconectado") {
      corDoStatus = isDark ? Colors.white : Colors.black; 
    } else if (WhatsappService.status.contains("ERRO") || WhatsappService.status.contains("CANCELADO") || WhatsappService.status.contains("TENTANDO")) {
        ...
        children: [
                  ...
                  child: Padding(
                    ...
                    child: Column(
                      ...
                      children: [
                          ...
                          GestureDetector(
                            onTap: () {
                              if (WhatsappService.pairingCode.isNotEmpty) {
                                ...
                                ScaffoldMessenger.of(context).showSnackBar(
                                  SnackBar(
                                    content: const Text('Código copiado!', style: TextStyle(color: Colors.white)), 
                            ...
                            child: Container(
                              ...
                              decoration: BoxDecoration(
                                color: corAcento.withOpacity(0.1), 
                                borderRadius: BorderRadius.circular(10), 
                                border: Border.all(color: corAcento.withOpacity(0.5), width: 2)
                              ),
                              child: Column(
                                children: [
                                  Text(
                        ...
                        if (WhatsappService.qrCodeImage != null && WhatsappService.pairingCode.isEmpty && !emSessao) ...[
                          const Divider(color: Colors.grey, height: 30),
                          Center(
                            child: Container(
                              color: Colors.white, 
                              padding: const EdgeInsets.all(10), 
                              child: Image.memory(WhatsappService.qrCodeImage!, height: 200)
                            )
                          ),
                        ]
                      ],
          ...
          if (!emSessao) ...[
            TextField(
              ...
              onSubmitted: (_) { 
                if (!_isLoading) _iniciarProcessoDeConexao(); 
              },
              decoration: InputDecoration(
                labelText: 'Seu WhatsApp (Ex: 12999999999)', 
                border: const OutlineInputBorder(), 
                prefixIcon: Icon(Icons.phone, color: !_isLoading ? corAcento : Colors.grey), 
                focusedBorder: OutlineInputBorder(borderSide: BorderSide(color: corAcento, width: 2))
              ),
            ),
            ...
            if (!_isLoading) 
              SizedBox(
                ...
                child: ElevatedButton(
                  onPressed: _iniciarProcessoDeConexao, 
                  style: ElevatedButton.styleFrom(backgroundColor: corAcento, foregroundColor: Colors.white), 
                  child: const Text('Conectar-se', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 16))
                )
              )
            else 
              SizedBox(
                ...
                child: ElevatedButton.icon(
                  ...
                  label: const Text('CANCELAR TENTATIVA', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 16))
                )
              ),
          ] else ...[
            Container(
              ...
              child: Row(
                children: [
                  Icon(Icons.phone, color: corAcento), 
...
class _DashboardTabState extends State<DashboardTab> {
  ...
  void _abrirForm(Viagem? v) {
    String dia = v?.dia ?? 'Segunda'; 
    String hIda = v?.horario ?? '8:00'; 
    String hVolta = '17:30';
      ...
      builder: (ctx) => StatefulBuilder(
        builder: (ctx, setM) => Padding(
          ...
          child: Column(
            ...
            children: [
              Text(
                v == null ? 'Nova Carona' : 'Editar Carona', 
                ...
                children: [
                  Expanded(
                    child: DropdownButtonFormField<String>(
                      value: hIda, 
                      items: horariosFixos.map((h) => DropdownMenuItem(value: h, child: Text(h))).toList(), 
                      onChanged: (val) => setM(() => hIda = val!), 
                      decoration: const InputDecoration(labelText: 'Ida', border: OutlineInputBorder())
                    )
                  ), 
                  ...
                  Expanded(
                    child: DropdownButtonFormField<String>(
                      value: hVolta, 
                      items: horariosFixos.map((h) => DropdownMenuItem(value: h, child: Text(h))).toList(), 
                      onChanged: (val) => setM(() => hVolta = val!), 
                      decoration: const InputDecoration(labelText: 'Volta', border: OutlineInputBorder())
                    )
                  ),
                ]
              ...
              ElevatedButton(
                ...
                onPressed: () {
                  if (v == null) { 
                    widget.viagens.add(Viagem(id: '${DateTime.now().millisecondsSinceEpoch}_i', dia: dia, tipo: 'Ida', horario: hIda)); 
                    widget.viagens.add(Viagem(id: '${DateTime.now().millisecondsSinceEpoch}_v', dia: dia, tipo: 'Volta', horario: hVolta)); 
                  } else { 
                    v.dia = dia; 
                    v.horario = hIda; 
                  }
              ...
              if (v != null) TextButton(
                onPressed: () { 
                  widget.viagens.remove(v); 
                  widget.onUpdate(); 
                  Navigator.pop(context); 
                }, 
                child: const Text('Excluir Carona', style: TextStyle(color: Colors.red))
              ), 
              const SizedBox(height: 20),
            ]
          )
  ...
  Widget build(BuildContext context) {
      ...
      grid.add(
        Padding(
          ...
          child: Row(
            crossAxisAlignment: CrossAxisAlignment.start, 
            children: [
              Expanded(child: _buildCard(widget.viagens[i])), 
              const SizedBox(width: 8),
              if (i + 1 < widget.viagens.length) 
                Expanded(child: _buildCard(widget.viagens[i + 1])) 
              else 
    ...
    return Scaffold(
      ...
      body: ListView(
        ...
        children: [
          const Text('Suas Caronas', style: TextStyle(fontSize: 24, fontWeight: FontWeight.bold)), 
          const SizedBox(height: 16), 
          ...grid
        ]
      ),
          ...
          children: [
            InkWell(
              ...
              child: Row(
                ...
                children: [
                  Text('${v.dia}\n${v.tipo} ${v.horario}', style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 13)), 
                  Text('(${v.passageiros.length})', style: TextStyle(color: corQtd, fontWeight: FontWeight.bold, fontSize: 16))
                ]
              )
            ), 
            const Divider(),
            ...v.passageiros.map((p) => Padding(
              padding: const EdgeInsets.symmetric(vertical: 2), 
              child: Row(
                children: [
                  Expanded(child: Text('• ${p.nome}', style: const TextStyle(fontSize: 12), overflow: TextOverflow.ellipsis)), 
                  GestureDetector(
                    onTap: () { 
                      setState(() => v.passageiros.remove(p)); 
                      widget.onUpdate(); 
                    }, 
                    ...
                    builder: (ctx) => AlertDialog(
                      ...
                      content: SizedBox(
                        ...
                        child: ListView.builder(
                          ...
                          itemBuilder: (ctx, idx) { 
                            return ListTile(
                              title: Text(widget.pessoasCadastradas[idx].nome), 
                              onTap: () { 
                                setState(() => v.passageiros.add(widget.pessoasCadastradas[idx])); 
                                widget.onUpdate(); 
                                Navigator.pop(context); 
                              }
                            );
  ...
  Widget build(BuildContext context) {
    return Scaffold(
          ...
          showModalBottomSheet(
            ...
            builder: (ctx) => Padding(
              ...
              child: Column(
                mainAxisSize: MainAxisSize.min, 
                children: [
                  const Text('Novo Contato', style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold)), 
                  const SizedBox(height: 16),
                  TextField(
                    controller: tN, 
                  ...
                  TextField(
                    controller: tT, 
                    keyboardType: TextInputType.phone, 
                    decoration: const InputDecoration(labelText: 'WhatsApp (Ex: 12999999999)', border: OutlineInputBorder())
                  ), 
                  const SizedBox(height: 20),
                  ElevatedButton(
                    style: ElevatedButton.styleFrom(minimumSize: const Size(double.infinity, 50)), 
                    onPressed: () { 
                      if(tN.text.isNotEmpty && tT.text.isNotEmpty) { 
                        pessoas.add(Pessoa(nome: tN.text, telefone: formatarTelefoneBr(tT.text))); 
                        onUpdate(); 
                        Navigator.pop(context); 
                      } 
                    }, 
      ...
      body: ListView(
        padding: const EdgeInsets.all(12), 
        children: [
          const Text('Seus Contatos', style: TextStyle(fontSize: 24, fontWeight: FontWeight.bold)), 
          const SizedBox(height: 10), 
          ...pessoas.asMap().entries.map((e) => ListTile(
            leading: const CircleAvatar(child: Icon(Icons.person)), 
            title: Text(e.value.nome), 
            subtitle: Text("+${e.value.telefone}"), 
            trailing: IconButton(
              icon: const Icon(Icons.delete_outline, color: Colors.red), 
              onPressed: () { 
                pessoas.removeAt(e.key); 
                onUpdate(); 
              }
            )
          )
...
class _ConfigTabState extends State<ConfigTab> {
  ...
  void _mostrarPreviaFiltro(BuildContext context, RegraMensagem regra) {
    ...
    String aviso = quemRecebe.isEmpty 
      ? "Previsão de Hoje: Ninguém estaria na carona alvo." 
      : "Se disparasse hoje, iria para: $nomes";
      
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(content: Text(aviso), duration: const Duration(seconds: 4), behavior: SnackBarBehavior.floating)
    );
  }
        ...
        builder: (ctx, setM) => Padding(
          ...
          child: SingleChildScrollView(
            child: Column(
              mainAxisSize: MainAxisSize.min, 
              children: [
                const Text('Configurar Mensagem', style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold)), 
                const SizedBox(height: 16),
                TextField(
                  controller: tM, 
                ...
                DropdownButtonFormField<String>(
                  value: tD, 
                  items: ['Dia Anterior', 'Dia da Carona', 'Fixo'].map((s)=>DropdownMenuItem(value: s, child: Text(s))).toList(), 
                  onChanged: (v)=>setM(()=>tD=v!), 
                  decoration: const InputDecoration(labelText: 'Quando?', border: OutlineInputBorder())
                ), 
                const SizedBox(height: 16),
                
                if (tD == 'Fixo') DropdownButtonFormField<String>(
                  value: tDFixo, 
                  items: ['Segunda', 'Terça', 'Quarta', 'Quinta', 'Sexta', 'Sábado', 'Domingo'].map((s)=>DropdownMenuItem(value: s, child: Text(s))).toList(), 
                  onChanged: (v)=>setM(()=>tDFixo=v!), 
                  decoration: const InputDecoration(labelText: 'Qual dia da semana?', border: OutlineInputBorder())
                ),
                if (tD == 'Fixo') const SizedBox(height: 16),
                
                DropdownButtonFormField<String>(
                  value: tAlvo, 
                  items: ['Ida', 'Volta'].map((s)=>DropdownMenuItem(value: s, child: Text(s))).toList(), 
                  onChanged: (v)=>setM(()=>tAlvo=v!), 
                ...
                DropdownButtonFormField<String>(
                  value: tH, 
                  items: ['Horário Específico', 'Minutos Depois'].map((s)=>DropdownMenuItem(value: s, child: Text(s))).toList(), 
                  onChanged: (v)=>setM(()=>tH=v!), 
                  decoration: const InputDecoration(labelText: 'Tipo de Horário', border: OutlineInputBorder())
                ), 
                const SizedBox(height: 16),
                TextField(
                  controller: tV, 
                  readOnly: tH=='Horário Específico', 
                  decoration: InputDecoration(
                    labelText: tH=='Horário Específico'?'Toque para escolher a hora':'Quantos minutos?', 
                    border: const OutlineInputBorder(), 
                    prefixIcon: const Icon(Icons.access_time)
                  ), 
                  onTap: tH=='Horário Específico' ? () async { 
                    TimeOfDay? p = await showTimePicker(context: context, initialTime: TimeOfDay.now()); 
                    if (p!=null) setM(()=>tV.text='${p.hour.toString().padLeft(2,'0')}:${p.minute.toString().padLeft(2,'0')}'); 
                  } : null
                ), 
                const SizedBox(height: 20),
                ElevatedButton(
                  ...
                  onPressed: () { 
                    if(tM.text.isNotEmpty && tV.text.isNotEmpty) { 
                      var n = RegraMensagem(
                        texto: tM.text, 
                        tipoDia: tD, 
                        diaFixo: tD == 'Fixo' ? tDFixo : null, 
                        tipoHorario: tH, 
                        valorHorario: tV.text, 
                        alvoCarona: tAlvo, 
                        ativo: rExistente?.ativo ?? true
                      );
                      if (idx != null) {
                        widget.regras[idx] = n; 
                      } else {
                      ...
                      widget.onUpdate(); 
                      Navigator.pop(context); 
                      if (n.ativo) _mostrarPreviaFiltro(context, n);
                    } 
                  }, 
                  child: const Text('Salvar')
                ), 
  ...
  Widget build(BuildContext context) {
    ...
    return ListView(
      ...
      children: [
        const Text('Automações de Mensagem', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: Colors.grey)), 
        const SizedBox(height: 10),
        Card(
          child: Column(
            children: [
                ...
                return ListTile(
                  leading: Switch(
                    ...
                    onChanged: (v) { 
                      r.ativo = v; 
                      widget.onUpdate(); 
                      if (v) _mostrarPreviaFiltro(context, r); 
                    }
                  ),
                  ...
                  trailing: PopupMenuButton(
                    onSelected: (val) async {
                      if (val == 'env') { 
                        ...
                        if (resultado == -1) {
                          ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Erro: Conecte o WhatsApp primeiro!'), backgroundColor: Colors.red));
                        } else if (resultado == -2) {
                          ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Erro API: Falta de sincronia. Tente Reconectar.'), backgroundColor: Colors.red));
                        } else if (resultado == 0) {
                          ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Aviso: Ninguém atende aos critérios para viagem HOJE.')));
                        } else {
                          ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Sucesso: Mensagem disparada para $resultado pessoa(s)!'), backgroundColor: Colors.green));
                        }
                      }
                      if (val == 'edit') {
                        _abrirConfigRegra(context, rExistente: r, idx: index);
                      }
                      if (val == 'del') { 
                    ...
                    itemBuilder: (ctx) => [
                      const PopupMenuItem(value: 'env', child: Text('Enviar agora')), 
              ...
              ListTile(
                ...
                title: const Text('Nova Mensagem', style: TextStyle(color: Colors.blue)), 
        ...
        const Text('Aparência', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: Colors.grey)), 
        const SizedBox(height: 10),
        Card(
          child: Column(
            children: [
              SwitchListTile(
                ...
                value: widget.isDark, 
                onChanged: (v) => widget.onThemeChanged(v, widget.currentColor)
              ),
              const Padding(
                padding: EdgeInsets.all(16), 
                child: Align(alignment: Alignment.centerLeft, child: Text('Cor de Destaque'))
              ),
              Padding(
                ...
                child: Row(
                  ...
                  children: cores.map((c) => GestureDetector(
                    onTap: () { 
                      widget.onThemeChanged(widget.isDark, c); 
                      if (c.value == Colors.greenAccent.value) { 
                        _clicks++; 
                        if (_clicks >= 3) { 
                          _clicks = 0; 
                          Navigator.push(context, MaterialPageRoute(builder: (ctx) => TelaSecretaLogs(regras: widget.regras, viagens: widget.viagens))); 
                        } 
                      } else { 
                        _clicks = 0; 
                      } 
                    ...
                    child: Container(
                      ...
                      decoration: BoxDecoration(
                        ...
                        border: widget.currentColor.value == c.value ? Border.all(color: widget.isDark ? Colors.white : Colors.black, width: 3) : null
                      )
                    )
                  )).toList()
                )
...
class TelaSecretaLogs extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    for (var r in regras) {
      ...
      if (r.tipoHorario == 'Horário Específico') {
        logs.add({'h': r.valorHorario, 't': r.texto});
      } else {
        int m = int.tryParse(r.valorHorario) ?? 0; 
        Set<String> listH = {};
        
        for (var v in viagens) {
          if (r.alvoCarona == v.tipo) {
            listH.add(formatarMinutos(parseMinutos(v.horario) + m));
          }
        }
        
        for (var h in listH) {
          logs.add({'h': h, 't': '${r.texto} [+$m min]'});
        }
      }
    }
    ...
    return Scaffold(
      ...
      body: logs.isEmpty 
        ? const Center(child: Text('Nenhum alarme engatilhado.', style: TextStyle(color: Colors.grey))) 
        : ListView.builder(
            itemCount: logs.length, 
            itemBuilder: (ctx, i) => ListTile(
              leading: const Icon(Icons.alarm, color: Colors.greenAccent), 
              title: Text(logs[i]['h'], style: const TextStyle(color: Colors.white, fontSize: 22, fontWeight: FontWeight.bold)), 
              subtitle: Text(logs[i]['t'], style: const TextStyle(color: Colors.grey))
            )
          )
    );
  }
